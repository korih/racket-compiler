...
(define/contract (flatten-program p)
  ...
  ;; interp. convert b expressions to flattened s expressions
  ...
  (define (flatten-program/tail tail)
    (match tail
      ...
      [`(begin ,fx ... ,tail)
       ;; block-asm-lang-v8.effect expressions are already para-asm-lang-v8.s expressions
  ...
  (match p
    [`(module ,bs ...)
     `(begin ,@(append-map flatten-program/b bs))]))
